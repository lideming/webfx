...
function hookFunction(obj, name, middleware) {
    const func = obj[name];
        ...
        hooked = function (...args) {
            const next = (index) => (...args) => {
                if (index < hooked.hookMiddlewares.length) {
                    return hooked.hookMiddlewares[index](next(index + 1)).apply(this, args);
                }
                else {
    ...
    hooked.hookMiddlewares.unshift(middleware);
    return hooked;
}
...
class View {
    constructor(dom) {
        ...
        this._mountState = MountState.Unmounted;
    ...
    addView(view, pos) {
        ...
        if (this._mountState == MountState.Mounted)
...
function injectCss(css, options) {
    var _a, _b;
    const parent = (_a = options === null || options === void 0 ? void 0 : options.parent) !== null && _a !== void 0 ? _a : document.head;
    parent.appendChild(buildDOM({ tag: (_b = options === null || options === void 0 ? void 0 : options.tag) !== null && _b !== void 0 ? _b : 'style', text: css }));
}
...
// file: viewlib.ts
function getWebfxCss() {
    return css;
}
let cssInjected = false;
function injectWebfxCss(options) {
    if (!cssInjected) {
        injectCss(getWebfxCss(), Object.assign(Object.assign({}, options), { tag: "style.webfx-injected-style" }));
        cssInjected = true;
    }
}
...
export { AutoResetEvent, BuildDOMCtx, ButtonView, Callbacks, CancelToken, ContainerView, ContextMenu, DataUpdatingHelper, Dialog, DialogParent, EditableHelper, EventRegistrations, FlagsInput, I, I18n, InputStateTracker, InputView, ItemActiveHelper, JsxNode, LabeledInput, LabeledInputBase, Lazy, LazyListView, ListView, ListViewItem, LoadingIndicator, MenuInfoItem, MenuItem, MenuLinkItem, MessageBox, MountState, Overlay, Ref, Section, SectionAction, SelectionHelper, Semaphore, TabBtn, TextBtn, TextCompositionWatcher, TextView, Timer, Toast, ToastsContainer, ToolTip, View, ViewToggle, addChild, appendView, arrayFind, arrayForeach, arrayInsert, arrayMap, arrayRemove, arraySum, base64EncodeUtf8, buildDOM, buildView, clearChildren, createArrayBuilder, createName, createStringBuilder, dragManager, fadeout, foreachFlaten, formatDateTime, formatDuration, formatFileSize, getDOM, getWebfxCss, hookFunction, i18n, injectCss, injectWebfxCss, jsx, jsxBuild, jsxFactory, listenEvent, listenEvents, listenPointerEvents, mod, mountView, numLimit, objectApply, objectInit, readBlobAsDataUrl, replaceChild, setPosition, sleepAsync, startBlockingDetect, strPadLeft, toggleClass, tryGetDOM, unmountView, version };
